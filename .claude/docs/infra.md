...
Vercel Cron (매일 자정 UTC)
      → GET /api/cron/daily (마감 리마인더 + 정기 결제 통합)
...
- 경로: `vercel.json` 참조
- 스케줄된 Cron 1개: `/api/cron/daily` — `0 0 * * *` (매일 자정 UTC)
  - 한 작업에서 **마감 리마인더 + Pro 구독 정기 결제**를 모두 처리한다 (`runDeadlineReminder` + 빌링키 청구).
  - Vercel Hobby 플랜의 Cron 1개 제한 때문에 `deadline-reminder`·`billing`을 `daily`로 통합했다. 두 라우트 핸들러(`/api/cron/deadline-reminder`, `/api/cron/billing`)는 코드에 남아 있으나 스케줄되지 않으며 수동/개별 호출용이다.
...
- [ ] Vercel Cron 1개(`/api/cron/daily`) 활성 상태 확인
