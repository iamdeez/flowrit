...
import { UpgradeModal } from '@/app/(dashboard)/settings/upgrade-modal'
...
export function ProjectForm({ customers, templates, members, defaultCustomerId }: ProjectFormProps) {
  ...
  const isPlanLimit = state?.error === 'PLAN_LIMIT_EXCEEDED:PROJECT'
  useFormToast(isPlanLimit ? null : state)

  if (isPlanLimit && !showUpgrade) {
    setShowUpgrade(true)
  }
