  ...
  async function handleUpgrade() {
    ...
    const handleReturnMessage = async (event: MessageEvent) => {
      ...
      if (data.type === 'NICEPAY_SUCCESS') {
        await processBilling(data.authToken as string, data.signature as string | undefined)
      } else {
    ...
    async function processBilling(authToken: string, signature?: string) {
      try {
        const res = await fetch('/api/billing/callback', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ authToken, signature, orderId, billingCycle }),
        })
    ...
    window.AUTHNICE.requestPay({
      ...
      fnSuccess: async (result) => {
        window.removeEventListener('message', handleReturnMessage)
        await processBilling(result.authToken, result.signData)
      },
      fnError: (result) => {
        window.removeEventListener('message', handleReturnMessage)
