...
- [ ] **T001** — recharts + shadcn/ui Chart 컴포넌트 설치
...
- [ ] **T002** `[P]` — `Project.budget` 필드 추가 + 마이그레이션
...
- [ ] **T003** — 기간 계산 유틸리티 + `isProjectDone` 확인 (T002 완료 후)
...
- [ ] **T004** `[P]` — `Project.budget` Server Action 추가 (T002 완료 후)
...
- [ ] **T005** — `app/(dashboard)/analytics/page.tsx` 신규 (T003 완료 후)
...
- [ ] **T006** — `completion-chart.tsx` 신규 — 월별 완료 바 차트 (T001, T005 완료 후)
...
- [ ] **T007** `[P]` — `revision-source-chart.tsx` 신규 — 수정 출처 파이 차트
...
- [ ] **T008** `[P]` — `inquiry-trend-chart.tsx` 신규 — 의뢰 추이 선 그래프
...
- [ ] **T009** `[P]` — `team-workload-table.tsx` 신규 — 팀 워크로드 표
...
- [ ] **T010** `[P]` — `period-selector.tsx` 신규 — 기간 선택
...
- [ ] `/analytics` 페이지가 정상 로딩되고 차트 4종이 모두 렌더링된다.
- [ ] 기간 필터 변경 시 데이터가 갱신된다.
- [ ] 프로젝트 생성·상세에서 예산 입력·저장이 가능하다.
- [ ] recharts 컴포넌트가 SSR 에러 없이 Client 컴포넌트로 동작한다.
