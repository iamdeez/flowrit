...
- [ ] **T001** — `Project.archivedAt` 필드 추가 + 마이그레이션
...
- [ ] **T002** — `lib/utils/csv.ts` 신규 — CSV 생성 유틸리티
...
- [ ] **T003** — `app/api/export/customers/route.ts` 신규 (T002 완료 후)
...
- [ ] **T004** `[P]` — `app/api/export/projects/route.ts` 신규 (T001, T002 완료 후)
...
- [ ] **T005** — `archiveProject`, `unarchiveProject` 구현 (T001 완료 후)
...
- [ ] **T006** `[P]` — `getProjects` 아카이브 필터 추가 (T001 완료 후)
  - 구현 파일: `lib/actions/project.ts`
  - 관련 요구사항: `FR-607`
  - 상세:
...
- [ ] **T007** `[P]` — 대시보드 카운트 쿼리 수정 (T001 완료 후)
...
- [ ] **T008** — `duplicateProject` 구현 (T001 완료 후)
  - 구현 파일: `lib/actions/project.ts`
...
- [ ] **T011** `[P]` — 프로젝트 상세 페이지 아카이브·복제 버튼 추가 (T005, T008 완료 후)
...
- [ ] **T012** `[P]` — `duplicate-project-dialog.tsx` 신규 (T008 완료 후)
...
- [ ] 고객·프로젝트 CSV 내보내기가 정상 동작하고 Excel에서 한글이 깨지지 않는다.
- [ ] 프로젝트 아카이브 시 목록에서 제거되고 대시보드 카운트에서 제외된다.
- [ ] 프로젝트 복제 시 단계가 복사되고 수정·에셋·타임라인은 비어 있다.
- [ ] 미인증 요청에 `/api/export/*`가 401을 반환한다.
