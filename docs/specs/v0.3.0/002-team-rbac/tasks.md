...
- [x] CHANGES.md에서 이전 작업(001-settings)의 "후속 작업 시 주의사항"을 확인했는가?
...
- [x] **T001** — `WorkspaceRole` 타입 정의 + 역할 헬퍼 함수 추가
...
- [x] **T002** — `inviteTeamMember` ADMIN 권한 검증 + 역할 선택 활성화 (T001 완료 후)
...
- [x] **T003** `[P]` — `cancelInvite` ADMIN 권한 검증 추가
  - 구현 파일: `lib/actions/team.ts`
  - 관련 요구사항: `FR-303`
...
- [x] **T004** — `changeMemberRole` 구현 (T001 완료 후)
  - 구현 파일: `lib/actions/team.ts`
...
- [x] **T005** `[P]` — `removeMember` 구현 (T001 완료 후)
  - 구현 파일: `lib/actions/team.ts`
...
- [x] **T006** `[P]` — `transferOwnership` 구현 (T001 완료 후)
  - 구현 파일: `lib/actions/team.ts`
  - 관련 요구사항: `FR-307`
  - 상세:
    - OWNER 권한 검증
...
- [x] **T007** — `app/(dashboard)/team/page.tsx` 수정 — ADMIN 뱃지 + 역할 변경 컨트롤 (T004~T006 완료 후)
...
- [x] **T008** `[P]` — `role-change-select.tsx` 신규
...
- [x] **T009** `[P]` — `remove-member-button.tsx` 신규
...
- [x] **T010** `[P]` — `transfer-ownership-button.tsx` 신규
...
- [x] **T011** `[P]` — `InviteForm` 역할 선택 드롭다운 추가
...
- [x] **T012** — SC-301~SC-309 수동 E2E 검증
...
- [x] ADMIN 역할이 DB에 저장되고 UI에 뱃지로 표시된다.
- [x] MEMBER 계정에서 초대·역할 변경·멤버 제거가 서버 측에서 차단된다.
- [x] `removeMember` 후 해당 사용자의 프로젝트 담당자 필드가 null로 변경된다.
- [x] 소유권 이전 후 구 OWNER는 ADMIN, 대상은 OWNER가 된다.
